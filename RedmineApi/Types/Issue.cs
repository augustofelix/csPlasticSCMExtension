...
namespace Redmine.Net.Api.Types
{
    ...
    public class Issue : Identifiable<Issue>, IXmlSerializable, IEquatable<Issue>, ICloneable
    {
        ...
        public void ReadXml(XmlReader reader)
        {
            ...
            while (!reader.EOF)
            {
                ...
                switch (reader.Name)
                {
                    ...
                    case "parent_issue_id":ParentIssueId = reader.ReadElementContentAsInt();break;
        ...
        public void WriteXml(XmlWriter writer)
        {
            ...
            // Added for Augusto Ortiz
            if (Journals != null)
            {

                if (Journals.Count == 1 && Journals[0].User == null)
                {
                    writer.WriteElementString("notes", Journals[0].Notes);
                }
                else
                {
                    writer.WriteStartElement("journals");
                    writer.WriteAttributeString("type", "array");

                    foreach (var jo in Journals)
                    {
                        new XmlSerializer(jo.GetType()).Serialize(writer, jo);
                    }
                    writer.WriteEndElement();
                }
            }

            // Added by Augusto Ortiz
            if (Changesets != null)
            {
                    ...
                    {
                        new XmlSerializer(cs.GetType()).Serialize(writer, cs);
                    }
                    writer.WriteEndElement();
            }
        }
